...
class GradientSlider extends React.Component {
	...
	onChange(value, stop_idx) {
		this.setState(prev_state => {
			return {
				stops: prev_state.stops.map((stop, i) =>
					i === stop_idx ? { ...stop, position: value } : stop
				)
			};
		});
	}
	...
	render() {
		...
		return (
			<div className="uix-gradient">
				...
				/>
				<MultiSlider onChange={this.onChange} onInsert={this.onInsert}>
					{stops.map((stop, idx) => (
						<MultiSliderHandle key={idx} value={stops[idx].position} property={idx} />
					))}
				</MultiSlider>
			</div>
		);
